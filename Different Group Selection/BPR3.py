...
parser.add_argument('--dataname', nargs='?', default='Yelp-4')
args = parser.parse_args()

dataname = args.dataname

#genre_count = pickle.load(open('./' + dataname + '/genre_count.pkl','rb'),encoding="latin1")#not used

train_df = pickle.load(open('./' + dataname + '/training_df.pkl','rb'),encoding="latin1")
vali_df = pickle.load(open('./' + dataname + '/valiing_df.pkl','rb'),encoding="latin1")  # for validation
# vali_df = pickle.load(open('./' + dataname + '/testing_df.pkl'))  # for testing
num_item = len(train_df['item_id'].unique())
num_user = len(train_df['user_id'].unique())

#key_genre = pickle.load(open('./' + dataname + '/key_genre.pkl','rb'),encoding="latin1")
key_genre = ["g1","g2","g3","g4"]

#item_idd_genre_list = pickle.load(open('./' + dataname + '/item_idd_genre_list.pkl','rb'),encoding="latin1")
...
#         lst = [0.5,0.05,0.15,0.3]
#         arr.append(np.random.choice(key_genre,p=lst))
#     arr = list(dict.fromkeys(arr))
#     item_idd_genre_list.append(arr)

# with open('./Yelp-4-randomgroup.pkl', 'wb') as handle:
#     pickle.dump(item_idd_genre_list, handle, protocol=pickle.HIGHEST_PROTOCOL)

#genre_item_vector = pickle.load(open('./' + dataname + '/genre_item_vector.pkl','rb'),encoding="latin1")
genre_item_vector = {}
for k in key_genre:
    genre_item_vector[k] = [[]]
    for i in range(num_item):
        if k in item_idd_genre_list[i]:
            genre_item_vector[k][0] += [1.0]
        else:
            genre_item_vector[k][0] += [0.0]
    genre_item_vector[k] = np.array(genre_item_vector[k])
        

#user_genre_count = pickle.load(open('./' + dataname + '/user_genre_count.pkl','rb'),encoding="latin1")
...
for u in range(num_user):
    ...
    for k in key_genre:
        o[k] = 0
    user_genre_count.append(o)
for index,row in train_df.iterrows():
    for genre in item_idd_genre_list[row["item_id"]]:
        user_genre_count[row['user_id']][genre] = user_genre_count[row['user_id']][genre]+1
    

for u in range(num_user):
    o = {}
    for k in key_genre:
        genre_count=len([a for a in genre_item_vector[k][0] if a>0])
        user_genre_count[u][k] = genre_count - user_genre_count[u][k]
